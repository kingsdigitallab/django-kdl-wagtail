...
class BasePerson(index.Indexed, ClusterableModel):
    ...
    @property
    def title(self):
        return "{} {}".format(self._title if self._title else "", self.name).strip()
...
def get_person_model():
    ...
    """
    try:
        return apps.get_model(settings.KDL_WAGTAIL_PERSON_MODEL, require_ready=False)
    except AttributeError:
        return Person
    except ValueError:
        raise ImproperlyConfigured(
            ("KDL_WAGTAIL_PERSON_MODEL must be of the form " '"app_label.model_name"')
        )
    except LookupError:
        raise ImproperlyConfigured(
            (
                'KDL_WAGTAIL_PERSON_MODEL refers to model "{}" '
                "that has not been installed".format(settings.KDL_WAGTAIL_PERSON_MODEL)
            )
        )
...
class PeopleIndexPersonRelationship(Orderable, models.Model):
    ...
    page = ParentalKey(
        "PeopleIndexPage",
        related_name="peopleindex_person_relationship",
        on_delete=models.CASCADE,
    )
    person = models.ForeignKey(
        PersonModel,
        related_name="person_peopleindex_relationship",
        on_delete=models.CASCADE,
    )
    panels = [SnippetChooserPanel("person")]
...
class PeopleIndexPage(BaseIndexPage):
    ...
    content_panels = BaseIndexPage.content_panels + [
        InlinePanel(
            "peopleindex_person_relationship", label="People", panels=None, min_num=1
        )
    ]

    search_fields = BaseIndexPage.search_fields + [
        index.RelatedFields(
            "peopleindex_person_relationship", [index.SearchField("person")]
        )
    ]

    def people(self):
        return self.peopleindex_person_relationship.all().select_related("person")
...
class PersonPage(BasePage):
    ...
    api_fields = BasePage.api_fields + [APIField("person")]

    content_panels = BasePage.content_panels + [SnippetChooserPanel("person")]
